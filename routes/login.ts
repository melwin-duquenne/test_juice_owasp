...
export function login () {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const email = typeof req.body.email === 'string' ? req.body.email.replace(/['";\\]/g, '').trim() : ''
    const password = typeof req.body.password === 'string' ? req.body.password.replace(/['";\\]/g, '').trim() : ''
    // Optionally, add stricter validation (e.g., regex for email)
    if (!email || !password) {
